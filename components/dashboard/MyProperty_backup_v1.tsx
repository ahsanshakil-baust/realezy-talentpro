...
const MyProperty = () => {
    ...
    const handleActionButton = async (actionLabel: any, propertyId: any) => {
        if (actionLabel.toLowerCase() === 'edit') {
            const property = properties.find((property: any) => property.id === propertyId)
            // console.log('booking', property)
            if (property.booking_status != 'reserve') {
                // if (property.isThread) {
                //   router.push(`/add-property-v3/${propertyId}?thread_status=${Buffer.from('threadCreated').toString('base64')}`)
                // } else if (!property.isThread) {
                //   // router.push(`/add-property-v3/${JSON.stringify(property)}`)
                router.push({
    ...
    return (
        <div className="db-page-layout">
            ...
            <div className="w-full bg-detailsCard py-8 px-12 rounded-3xl min-h-[70vh]">
                ...
                {console.log('availablProperties', availablProperties)}
                {/* <button
