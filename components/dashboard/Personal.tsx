...
const Personal = () => {
  ...
  const { data: session, update }: any = useSession() // , status
  ...
  const [editView, setEditView] = useState(router?.query?.complePro ? true : router?.query?.hp ? true : false)
                  ...
                  {session && session?.user?.userInfo?.isOfferable && (
                    <>
                      <div className=" flex gap-5 md:gap-[1.5rem] 2xl:gap-[1.875rem] mb-5 md:mb-[1.5rem] 2xl:mb-[1.875rem]">
                        <Button
                          variant="contained"
                          className={`${isTenant(type) ? 'bg-[#00ADEE]' : 'bg-[#034EA1]'}`}
                          onClick={calculateViewRentalRange}>
                          View Rental Range
                        </Button>
                      </div>
                    </>
