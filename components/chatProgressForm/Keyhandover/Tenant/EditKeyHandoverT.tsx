...
const EditKeyHandoverT = () => {
  ...
  const keyHandoverEditHandler = (_: any) => {
    // const formData = {
    //   id: 'keyId',
    ...
    /**
        ...
        const updatedKeyLandlordData = {
                selected_time: formatTime,
                inviterId: userId,
                id: id?.toString(),
                time: parseInt(getTimestamp()),
                selected_date: formDate,
                status:
                    context.value.currentRoleType == "isLandlord"
                        ? `Landlord Changed Schedule`
                        : `Tenant Changed Schedule`,
  ...
  return (
    <div className="w-[800px] bg-blue-400 p-5">
      <form onSubmit={handleSubmit(keyHandoverEditHandler)}>
        <div className="flex gap-5">
          ...
          <div className="w-2/5 border border-blue-300 rounded-md shadow-md">
            <input
              ...
              id="key_handover"
              {...register('key_handover')}
              defaultValue="key_handover"
              // checked={ data?.data?.appointmentInfo.appointment_purpose == "key_handover" ? true : false }
            />
...
export default EditKeyHandoverT
