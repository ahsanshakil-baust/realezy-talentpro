...
import PoiWarning from '../../components/propertyDetails/PoiWarning'
...
const PropertyAdd: NextPageWithLayout = () => {
    ...
    const [monthlyRental, setMonthlyRental] = useState(propertyData?.rental_amount ? propertyData?.rental_amount : '')
    ...
    const [availableFrom, setAvailableFrom] = useState(
        propertyData?.available_from ? propertyData?.available_from : new Date()
    )
    ...
    // const [hdbestate, setHdbestate] = useState('') // estate
    ...
    const [createProperty] = useCreatePropertyMutation() // , { isError: propertyCreateError, isLoading: propertyCreateIsLoading, data: propertyData }
    ...
    //FEATCHING PROPERTY DATA BY PROPERTY ID
    // const { data: propertyData, isLoading } = useGetPropertyDetailsQuery(propertyID as string)

    // console.log('isLoading_data', propertyData)
    //!============================
    ...
    const [uploadFiles] = useMultiFileUploadMutation() // , { isError: uploadFilesIsError, isLoading: uploadFilesIsLoading, data: uploadedFiles }
    ...
    const handlePostalCode = async (e: any) => {
        ...
        if (
            ...
            if (data?.status === 'success') {
                ...
                setAddress(
                    `${data?.Postcodes[0]?.BuildingNumber}${data?.Postcodes[0]?.BuildingNumber !== '' ? ', ' : ''}${data?.Postcodes[0]?.BuildingName
                    ...
                    <div className="w-[90%] md:w-full m-auto ">
                        <div className="  w-full flex flex-col sm:flex-row gap-3 sm:items-center justify-between pt-3 xl:pt-4 2xl:pt-6 pb-3 xl:pb-4 2xl:pb-5 ">
                            ...
                            <div className="flex  items-center gap-2 sm:gap-3 md:gap-4 xl:gap-6">
                                ...
                                <Link passHref href={propertyID ? '/dashboard/my-properties' : '/'}>
                                    <Button
                                        type="button"
                                        variant="outlined"
                                        ...
                                        <div className="!grid !grid-cols-2 !gap-7 !mb-3 md:!mb-4 2xl:!mb-5  ">
                                            ...
                                            <div>
                                                <DatePicker
                                                    ...
                                                    defaultValue={moment(availableFrom)}
                                    ...
                                    <div>
                                        ...
                                        {previewTaxImage?.length > 0 ? (
                                            previewTaxImage?.map((item, index) => (
                                                <div key={index} className=" relative mt-8 mb-2">
                                        ...
                                        {previewTaxImage?.length == 0 && <div className="bg-white px-4 md:px-6 xl:px-8 2xl:px-10  py-3 md:py-4 xl:py-5 2xl:py-6 rounded-t-[10px] ">
